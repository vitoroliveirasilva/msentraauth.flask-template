...
from .infrastructure.session_backend import SessionBackendUnavailable
...
def register_error_handlers(app: Flask) -> None:
    ...
    def bad_request(error: BadRequest) -> Response | tuple[str, int]:
        if isinstance(error, SecurityError):
            return app.response_class("Bad Request\n", status=400, mimetype="text/plain")
    ...
    def session_backend_unavailable(error: SessionBackendUnavailable) -> Response:
        current_app.logger.error(
            "server-side session backend unavailable",
    ...
    def internal_error(_: object) -> tuple[str, int]:
        return _render(
            "Falha interna",
            "A operação não pôde ser concluída. Informe o ID da requisição ao suporte.",
            500,
        )
